...
(deftest orbit-proxy-satisfies-expr-uses-t
  (is (expr-uses-t? 'ecc-anom))
  (is (expr-uses-t? '(e/cos ecc-anom)))
  (is (expr-uses-t? '(e/sinh hyp-anom)))
...
(deftest ellipse-conic-plus-hyperbola-template-fitness
  (let [ds (scenarios->datasets default-scenarios)]
    (binding [*both-regimes?* true
              *template-unbound-arms?* true
              *template-conic-unbound?* true
              *analytical-blocks?* true]
      (let [ell-wrapped (#'core/wrap-catalog-entry-strict (blocks/catalog-entry :ellipse-conic))
            fit (#'core/calculate-analytical-de-driven-fitness ell-wrapped ds)]
        (is (analytical-strict-energy-branches? ell-wrapped))
        (is (#'core/expr-uses-op? (unbound-arm-expr :qx-expr) 'e/sinh))
        (is (pos? fit))))))

(deftest conic-catalog-matches-oracle
