...
Example run focused on conserved invariants with physics-driven fitness:
...
| `--strategy NAME` | Restrict immigrants to `analytical` or `conserved` (default: composite analytical + conserved) |
...
By default, new individuals are composite genomes containing both one `:analytical`
law and one `:conserved` law. Use `--strategy analytical` or
`--strategy conserved` only for focused single-law runs.
...
**Graded primitives:** `--primitive-tier auto|N` unlocks orbital pipeline stages (`e/mean-anomaly` → `e/anomaly` → periapsis → lab frame). **`--primitive-pipeline-only`** forces tier 5, bans oracle leaf symbols (`ecc-anom`, `hyp-anom`, `mean-M`, `peri-xp`, `peri-yp`) from GP search, and disables conic catalog/template shortcuts — position must be built from graded primitives while the unified oracle still supplies compile-time elements (`ecc`, `n-mean`, `peri-vxp`, …).

**Both regimes (default for `--de-driven --strategy analytical`):** fitness uses **all** reference scenarios; `:domain` tags are ignored; every analytical slot must be `(e/if (neg? energy) bound-arm unbound-arm)` with no bogus `e/if` tests. **Per-regime arm fitness:** bound arms are scored only on bound scenarios, unbound arms on unbound; overall fitness is `min(bound, unbound)`. **Template unbound (default):** with analytical blocks, unbound arms use **`:hyperbola-conic`** (`e/sinh`/`e/cosh` on `hyp-anom`); otherwise Taylor. Bound arms co-evolve in `(qx,px)` and `(qy,py)` pairs. **Analytical blocks (default):** Emmy-validated catalog laws inject via immigrants, guess mutations, and pair grafts. Opt out with `--no-template-unbound` or `--no-analytical-blocks`. Use `--domain-filter` for legacy domain-tagged mode.

With `--de-driven`, analytical laws score trajectory fit at **20% and full-orbit** horizons on integrated reference orbits (fitness = min of the two); conserved laws score **temporal constancy along integrated orbits** (they are invariants, not solutions of the motion DE).
